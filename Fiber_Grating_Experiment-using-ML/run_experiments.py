...
1. Standard Master Benchmark (LR, RF, SVR, MLP, GPR, PINN)
2. Comprehensive Multi-Model Noise Robustness (1% - 10% Noise)
3. Comprehensive Multi-Model Small Data Regime (20% - 100% Data)
4. Physics Weight Lambda Ablation Study
5. PINN Hyperparameter Sensitivity Study
6. Leak-Free 5-Fold Cross Validation & Bootstrap Analysis
7. Literature FBG Methods Comparison Matrix
8. Publication-Quality Figure Suite Generation (300 DPI PNGs)
=============================================================================
"""
...
from src.experiments.sensitivity import run_hyperparameter_sensitivity_study
from src.experiments.statistical_validation import run_cross_validation, run_bootstrap_analysis
from src.experiments.literature_comparison import generate_literature_fbg_comparison
...
def main():
    ...
    # 2. Comprehensive Noise Robustness across ALL 6 Models
    ...
    # 3. Comprehensive Small Data Regime across ALL 6 Models
    ...
    df_sens = run_hyperparameter_sensitivity_study()
    
    # 6. Leak-Free 5-Fold Cross-Validation & Bootstrap Analysis across ALL 6 Models
    df_cv, fold_results = run_cross_validation(n_splits=5)
    plot_cross_validation_boxplot(fold_results)
    
    df_boot = run_bootstrap_analysis(n_bootstraps=200)
    
    # 7. Literature Comparison Matrix
    df_lit = generate_literature_fbg_comparison()
    
    # 8. Publication Figures (GPR Uncertainty, Parity Plots, Residuals)
    print("\nGenerating Publication Figures for GPR Uncertainty, Parity Plots & Residuals...")
    ...
    mlp = MLPModel().fit(X_tr_s, y_tr)
    y_pred_mlp = mlp.predict(X_te_s)
    
    plot_prediction_residuals(y_te, y_pred_pinn, y_pred_gpr)
    plot_parity_plots(y_te, y_pred_pinn, y_pred_mlp)
    
    print("\n" + "=" * 70)
    print("ALL EXPERIMENTS, STATISTICAL TABLES & FIGURES COMPLETED SUCCESSFULLY!")
